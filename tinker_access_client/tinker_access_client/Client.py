...
class Client(Machine):
    ...
    def __show_remaining_time(self):
        ...
        temperatureC, temperatureF = self.__device.read(Channel.TEMPERATURE)
        m, s = divmod(int(remaining_seconds), 60)
        h, m = divmod(m, 60)
        user_name = self.__user_info.get('user_name')
		
        if (temperatureC == DeviceApi.NO_TEMPERATURE_INDICATOR):
           	line2 = '{0:02d}:{1:02d}:{2:02d}'.format(h, m, s).center(maximum_lcd_characters, ' ')
        else:
            line2 = '{0:02d}:{1:02d}:{2:02d} - {3:02.1f}C'.format(h, m, s, temperatureC).center(maximum_lcd_characters, ' ')
...
myclient = Client(2, opts)
print opts
print "Starting"
print Channel.LCD
print Channel.TEMPERATURE
myclient.run(opts,args)